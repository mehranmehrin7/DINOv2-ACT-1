...
def train_target(args):
    ...
    if args.net == 'dinov2':
        netC1 = network.Classifier(
            feature_dim=netF.in_features,
            bottleneck_dim=args.bottleneck,
            class_num=args.class_num
        ).cuda()
        netC2 = network.Classifier(
            feature_dim=netF.in_features,
            bottleneck_dim=args.bottleneck,
            class_num=args.class_num
        ).cuda()

        src_netC1 = network.Classifier(
            feature_dim=src_netF.in_features,
            bottleneck_dim=args.bottleneck,
            class_num=args.class_num
        ).cuda()
        src_netC2 = network.Classifier(
            feature_dim=src_netF.in_features,
            bottleneck_dim=args.bottleneck,
            class_num=args.class_num
        ).cuda()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--net', type=str, default='resnet50', help="alexnet, vgg16, resnet50, resnet101, deit, dinov2")
